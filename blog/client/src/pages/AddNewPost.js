...
function AddNewPost({ addNewPost, tagsList }) {
  ...
  const onSubmitHandler = async (event) => {
    ...
    const response = await addNewPost(uuidv4(), title, content, selectedTag);
    if (response === 'success') {
      navigate('/');
    }
  }
  ...
  return (
    <div className='container'>
      <Card component='form' className='form' onSubmit={(event) => onSubmitHandler(event)} data-testid='addNewPost-card'>
